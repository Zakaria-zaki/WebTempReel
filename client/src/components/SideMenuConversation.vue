<template>
        <li class="my-px">
            <router-link
              :to="{ name: 'messages', params: { receiverId: receiverId }, query: { name: fullName }}"
              class="flex flex-row items-center h-10 px-3 rounded-lg text-gray-300 hover:bg-gray-100 hover:text-gray-700"
            >
              <div class="relative">
                  <span v-if="connected" class="absolute text-green-500 right-0 bottom-0">
                    <svg width="20" height="20">
                        <circle cx="15" cy="15" r="5" fill="currentColor"></circle>
                    </svg>
                  </span>
                  <div
...
<script>
export default {
    props: ['fullName', 'connected', 'receiverId']
}
</script>